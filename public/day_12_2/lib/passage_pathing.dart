...
extension PathExtensions on Path {
  ...
  bool get isValid {
    final times = this
        .where((item) => !item.isBigCave && item != 'end' && item != 'start')
        .toSet()
        .map((small) => this.where((e) => e == small).length)
        .toList();
    if (times.any((element) => element > 2)) return false;

    final amount = times.where((times) => times > 1).length;
    return amount < 2;
  }
}
